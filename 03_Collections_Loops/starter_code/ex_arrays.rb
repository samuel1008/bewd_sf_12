...
#push adds to the end, shift adds to the beginning, << same as push
...
categories << "Animals"
#categories.include? ("Animals" && "Shopping" ) is not correct -> this will make Animals true, shopping true, categories include (true)
if categories.include?("Animals") && categories.include?("Shopping")
  categories << "Family"
  #if animals = true, shopping = false, if true&&false/false = true
end
...
# sort! will change the catagory, .sort will keep catagory the same, but will sort it for you
#join(", space")
